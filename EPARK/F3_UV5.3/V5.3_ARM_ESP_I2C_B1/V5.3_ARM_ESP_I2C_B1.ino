...
void setup(void)
{
  ...
  */
  ...
  if ( screen == 40) {
    ...
    tft.fillScreen(BLACK);
    showmsgXY(1, 100, 2, &FreeSans9pt7b, "The number you \nentered cannot be \nused as PIN for you please go back and enter again") ;
    tft.setFont(NULL);
    Back.initButton(&tft,  41 - 7, 460 + 5, 80, 30, ILI9481_RED, ILI9481_RED, BLACK, "Back", 2);
    Back.drawButton(false);
    Serial.println("SETUP DONE NOW MOVING TO LOOP AND FURTHER LOOP") ;
      ...
      if (buttons[b].justPressed()) {
        ...
        if (b == 0) {
          ...
          if ( textfield_i != 4 ) {
            ...
            for ( iii = 0 ; iii < 4 ; iii++) {
              for ( jjj = 0 ; jjj < 5 ; jjj++) {
                if (c[iii][jjj].number[0] == textfield[0] && c[iii][jjj].number[1] == textfield[1] && c[iii][jjj].number[2] == textfield[2] && c[iii][jjj].number[3] == textfield[3]) {
