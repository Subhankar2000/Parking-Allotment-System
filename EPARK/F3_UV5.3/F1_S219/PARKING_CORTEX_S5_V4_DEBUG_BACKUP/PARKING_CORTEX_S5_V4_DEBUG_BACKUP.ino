...
void setup(void)
{
  ...
  */
  ...
  if ( screen == 5 ) {
    ...
    Serial.println("NOW ENTERING SETUP FOR SCREEN 5") ;
    Serial.println("SCREEN 5 - CONFIRMATION SCREEN WHERE THE USER HAS TO PRESS FINISH IN ORDER TO KEEP VEHICLE INSIDE") ;
...
void loop(void)
{
  ...
  if (screen == 145) {
    Serial.println("ENTERING LOOP FOR SCREEN 145");
    ...
    if (no.justPressed()) {
      no.drawButton(true);
      c[i][counter[i]].yes = 9 ;
      Serial.println("LEAVING SCREEN 145") ;
      screen5() ;
    }
  }

  if (screen == 1) {
    Serial.println("ENTERING SCREEN 1 LOOP") ;
    ...
    if (one.justPressed()) {
      one.drawButton(true);
      Serial.println("ONE PRESSED # JUMPING TO SCREEN 2") ;
      screen2() ;
    }
    if (two.justPressed()) {
      two.drawButton(true);
      Serial.println("TWO PRESSED # JUMPING TO SCREEN 21") ;
      screen21() ;
    }
  }
  ...
  if (screen == 2) {
    ...
    Serial.println("ENTERING LOOP FOR SCREEN 2") ;
    bool down = Touch_getXY();
    ...
    if (!( counter[0] == 5 && counter[1] == 5 && counter[2] == 5 && counter[3] == 5 )) {
      if (Ok.justReleased())
        Ok.drawButton();
      if (Ok.justPressed()) {
        Serial.println("OK PRESSED # JUMP TO SCREEN 3") ;
        Ok.drawButton(true);
        screen3() ;
      }
    }
    ...
    if (Back.justPressed()) {
      Serial.println("BACK PRESSED # JUMP TO SCREEN 1 PREVIOUS SCREEN") ;
  ...
  if (screen == 23) {
    Serial.println("ENTERING LOOP FOR SCREEN 23") ;
    bool down = Touch_getXY();
    Ok.press(down && Ok.contains(pixel_x, pixel_y));
    Back.press(down && Back.contains(pixel_x, pixel_y));
    ...
    if (Ok.justPressed()) {
      ...
      //        tempT2 = -1 ;
      //      }
      Serial.println("GOING TO SCREEN 4 ANYWAYS") ;
    }
    if (Back.justPressed()) {
      Back.drawButton(true);
...
Serial.println("BACK PRESSED GOING BACK TO SCREEN 3") ;
      screen3() ;
    }
  ...
  if (screen == 3 ) {
    Serial.println("ENTERING LOOP FOR SCREEN 3") ;
    bool down = Touch_getXY();
    Ok.press(down && Ok.contains(pixel_x, pixel_y));
    Back.press(down && Back.contains(pixel_x, pixel_y));
    ...
    if (Ok.justPressed()) {
      ...
      Serial.println("OK PRESSED # JUMP TO SCREEN 23") ;
    }
    if (Back.justPressed()) {
      Back.drawButton(true);
      temp1 = -1 ;
      ...
      i = -1 ;
Serial.println("BACK PRESSED # JUMP TO SCREEN 2") ;
      screen2() ;
    }
  }

  if (screen == 4) {
    Serial.println("ENTERING LOOP FOR SCREEN 4") ;
    bool down = Touch_getXY();
    ...
    if ( n == 1 ) {
      ...
      if (Ok.justPressed()) {
        ...
        for ( k = 0 ; k < TEXT_LEN  ; k++)
          textfield[k] = ' ' ;
        textfield_i = 0 ;
        Serial.println("JUMPING TO SCREEN 145") ;
        screen145() ;
      }
    }
    if (Back.justPressed()) {
