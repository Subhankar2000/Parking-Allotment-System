/*
     ...
     PROJECT MADE AFTER THE FIRST PROJECT NAMED - "VEHICLE PARKING MANAGEMENT"
...
int n = 0 , nr = 0 , tempT1 = -1 , tempT2 = -2 , counter[4] , temp1 = -1 , temp2 = -2 , temp3 = -3 , temp4 = -4 , found = -1 , mfound = -1;
...
void setup(void)
{
  ...
  */
  ...
  if ( screen == 22) {
    ...
    tft.setTextColor(WHITE);
    tft.setTextSize(1);
    tft.print( (((rtc.getUnixTime(rtc.getTime())) % 10000000) - c[i][j].sTime) / 3600  );
    tft.setFont(NULL);
    ...
    if ( c[i][j].type == 2 ) {
      ...
      tft.setTextColor(WHITE);
      tft.setTextSize(1);
      tft.print( (((rtc.getUnixTime(rtc.getTime())) % 10000000) - c[i][j].sTime) * ccm  );
      tft.setFont(NULL);

    }
    else if ( c[i][j].type == 1 ) {
      a = (((rtc.getUnixTime(rtc.getTime())) % 10000000) - c[i][j].sTime) ;
      tft.setFont(&FreeSans12pt7b);
      tft.setCursor(1 + 120, 250 + 25 + 40 + 40 + 20);
      tft.setTextColor(WHITE);
      tft.setTextSize(1);
...
void loop(void)
{
  ...
  if (screen == 5) {
    ...
    if (Finish.justPressed()) {
      ...
      screen1() ;
      c[i][counter[i]].sTime = (rtc.getUnixTime(rtc.getTime())) % 10000000 ;
    ...
    for (uint8_t b = 0; b < 15; b++) {
      ...
      if (buttons[b].justPressed()) {
        ...
        if (b == 0) {
          status(F("Ok"));
          n = 1 ;
          textFT = String(textfield) ;
          c[i][counter[i]].number = textFT ;
          screen4() ;
        }
