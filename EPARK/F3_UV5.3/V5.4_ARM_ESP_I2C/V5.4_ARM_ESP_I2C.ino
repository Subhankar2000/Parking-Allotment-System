/*   EPARK CORTEX S5 [ MINI / GSM / SERVER(b) ] <----------------------------------------------------------------------------------> [ V1 / V5 / V5.3(b) ]
...
/*    FUNCTIONS FOR CHANGING VALUES OF SETUP AND LOOP SO THAT THE SPECIFIC SCREEN GETS INITIALIZED AND GETS OPEN
      ...
      SCREEN 22 - FINAL CONFIRMATION SCREEN WHERE THE USER PRESSES FINISH AND TAKES THE VEHICLE OUT
      SCREEN 22_0 - 220 - THE ERROR SCREEN WHERE THE USER WILL GET WHEN ENTERED THE WRONG VEHICLE'S NUMBER OR WHICH IS NOT IN THE DATABASE
      SCREEN 31 - OTP SCREEN
      SCREEN 200913 - NUMPAD SCREEN FOR OTP
      SCREEN 20022_03 - NOT FOUND SCREEN FOR OTP
...
void setup(void)
{
  ...
  */
  ...
  if ( tft_first == 1 ) {
    Serial.println("EPARK CORTEX S5 [ MINI / GSM / SERVER(b) ] <----------------------------------------------------------------------> [ V1 / V5 / V5.3(b) ]") ;
  ...
  */
  ...
  if ( screen == 9999) {
    Serial.println("NOW ENTERING THE SCREEN 9999");
    Serial.println("MASTER PIN");
    ...
    for ( iiii = 0 ; iiii < 4 ; iiii++) {
      for ( jjjj = 0 ; jjjj < 5 ; jjjj++) {
        for ( iiiii = 0 ; iiiii < 7 ; iiiii++)
          tft.print(c[iiii][jjjj].number[iiiii]) ;
      }
      tft.print("\n") ;
    }
        ...
        if (b == 0) {
          if ( textfieldr_i != TEXT_LEN ) {
            ...
            screen21();
              ...
              if ( textfieldr[0] == secret[0] && textfieldr[1] == secret[1] && textfieldr[2] == secret[2] && textfieldr[3] == secret[3] && textfieldr[4] == secret[4]  && textfieldr[5] == secret[5] &&  textfieldr[6] == secret[5] )
              screen9999() ;
